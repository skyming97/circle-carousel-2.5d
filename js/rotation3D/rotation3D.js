...
//         const rotate3D = document.querySelector(".rotation3d-wrap");
//         const currentClientWidth = document.documentElement.clientWidth;
//         const rotate3DW = rotate3D.offsetWidth;
//         rotate3D.style.transform = `scale(${(currentClientWidth) / rotate3DW})`;
//     }
//     handleResize()
...
var circleMath = {
    /**
     * 根据弧度计算角度 
     *  1度对应的圆周长即弧度大小。例如：2 * Math.PI * r / 360 = 1度对应的圆周长 = 弧度 （半径r=1)
     * @param rotation 弧度
    ...
    parseRotate: function (rotation, self) {
        ...
        // 输入rotation弧度，转换对应rotation弧度的角度数（-180含义是从0度开始）
...
Rotation3D.prototype.goTo = function (index) {
    ...
    console.log('index curIndex', index, this.currentIndex)
    /**
    ...
    console.log('this.destRotation', this.destRotation)
...
Rotation3D.prototype.render = function () {
    ...
    this.$line.each(function (index, item) {
        item.style.opacity = 0.6
        self.lineStyle($(this), index, lineRotation)
        lineRotation += spacing;
    })
    // 高亮当前元素
    const curItemEle = this.$item[this.currentIndex]
    curItemEle.style.opacity = 1
}
/**
 * 自动播放
 */
Rotation3D.prototype.onAutoPlay = function () {
    ...
    if (this.autoPlay) {
        this.autoPlayTimer = setInterval(function () {
            ...
            console.log('curIndex', self.currentIndex)
        }, this.autoPlayDelay)
    }
}
/**
...
Rotation3D.prototype.onDrag = function () {
    ...
    function play() {
        ...
        self.goTo(self.currentIndex % self.length);
    }
